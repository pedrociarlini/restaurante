...
public class JTicketLines extends javax.swing.JPanel {
    ...
    public JTicketLines(AppView oApp) {
        
        initComponents();
  
        ColumnTicket[] acolumns = new ColumnTicket[0];
        String sTicketLine = oApp.lookupDataLogic(DataLogicSystem.class).getResourceAsXML("Ticket.Line");
        if (sTicketLine != null) {
            try {
                if (m_sp == null) {
        ...
        // la tabla.        
        m_jTableModel = new TicketTableModel(oApp, acolumns);
...
//        }       
    ...
    private static class TicketTableModel extends AbstractTableModel {
        ...
        public void setRow(int index, TicketLineInfo oLine){
            ...
            for (int i = 0; i < m_acolumns.length; i++) {
               row[i] = m_App.evaluateTemplate(m_acolumns[i].value, vc);    
