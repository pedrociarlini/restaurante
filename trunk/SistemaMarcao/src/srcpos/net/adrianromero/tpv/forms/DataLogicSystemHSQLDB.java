...
public class DataLogicSystemHSQLDB extends DataLogicSystem {
    ...
    public void init(Connection cnt) {
        ...
        m_sInitScript = "/net/adrianromero/tpv/data/scripthsqldb";

        m_peoplevisible = new StaticSentence(cnt
            , "SELECT NAME, APPPASSWORD, ROLE, IMAGE FROM PEOPLE WHERE VISIBLE = TRUE"
            , null
            , new SerializerReadBasic(new Datas[] {Datas.STRING, Datas.STRING, Datas.STRING, Datas.IMAGE}));   

        m_nextclosedcash = new StaticSentence(cnt, "CALL NEXT VALUE FOR CLOSEDCASHNUM", null, SerializerReadInteger.INSTANCE);                       
