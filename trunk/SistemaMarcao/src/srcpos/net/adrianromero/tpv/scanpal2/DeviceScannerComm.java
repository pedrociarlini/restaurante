...
//    private static final int STATUS_READING = 2;
    ...
    public void connectDevice() throws BasicException {    
...
//            e.printStackTrace();
    ...
    public void startDownloadProduct() throws BasicException {
        writeLine(COMMAND_READ); // writeLine(COMMAND_READ);
        readCommand(COMMAND_ACK);
    }
    public ProductDownloaded recieveProduct() throws BasicException {
        byte[] line = readLine();
    ...
    public void startUploadProduct() throws BasicException {
        // Inicializamos la conversacion
        writeLine(COMMAND_CIPHER);
        readCommand(COMMAND_ACK);      
        m_iProductOrder = 0;
    }
    public void sendProduct(String sName, String sCode, Double dPrice) throws BasicException {
    ...
    public void stopUploadProduct() throws BasicException {
        // Cerramos la conversacion
        writeLine(COMMAND_OVER);
        readCommand(COMMAND_ACK);
    }
    
    private void readCommand(byte[] cmd) throws BasicException {
        byte[] b = readLine();
        if (!checkCommand(cmd, b)) {
            // excepcion que te crio.
            throw new BasicException("Command not expected");
        }
    }
    
    private void writeLine(byte[] aline) throws BasicException {
        
        if (m_CommPortPrinter == null) {
            throw new BasicException("No Serial port opened");
        } else {
    ...
    private byte[] readLine() throws BasicException {
        ...
        synchronized (this) {
            ...
            if (m_aLines.isEmpty()) {  
                throw new BasicException("Timeout");
            } else {
                return m_aLines.poll();
            }            
        }
