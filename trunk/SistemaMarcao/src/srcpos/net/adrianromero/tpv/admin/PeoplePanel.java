...
public class PeoplePanel extends JPanelTable {
    ...
    public PeoplePanel(AppView oApp) {
        ...
        TableDefinition tpeople = m_App.lookupDataLogic(DataLogicAdmin.class).getTablePeople();
        
        ListProvider lpr = new ListProviderECreator(tpeople);
        SaveProvider spr = new SaveProvider(tpeople);        
        
        jeditor = new PeopleView(m_App);    
        BrowsableEditableData bd = new BrowsableEditableData(lpr, spr, new EditorRecordBasic(jeditor));
                
        // el centro
        addEditor(jeditor);
        
        initNavigation(bd, tpeople.getVectorerBasic(), tpeople.getComparatorCreator(), tpeople.getRenderStringBasic());
    }
