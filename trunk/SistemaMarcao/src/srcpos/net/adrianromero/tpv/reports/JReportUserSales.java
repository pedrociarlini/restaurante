...
public class JReportUserSales extends JPanelReport {
    ...
    public JReportUserSales(AppView oApp) {
        super(oApp);
    }
    ...
    protected String getReport() {
        return "/net/adrianromero/reports/usersales";
    }
    ...
    protected BaseSentence getSentence() {
        return new StaticSentence(m_App.getConnection()
            , new QBFBuilder(
                "SELECT PEOPLE.NAME, MIN(TICKETS.DATENEW) AS STARTDATE, MAX(TICKETS.DATENEW) AS ENDDATE, SUM(PRODUCTSOUT.UNITS * PRODUCTSOUT.PRICE) AS TOTAL " +
                "FROM PEOPLE, TICKETS, PRODUCTSOUT WHERE PEOPLE.NAME = TICKETS.PERSON AND TICKETS.TICKETID = PRODUCTSOUT.TICKETID AND ?(QBF_FILTER) " +
